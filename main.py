...
# Single-file Streamlit app | $0 Capital Stack | gemini-3.1-flash-lite
...
# ─────────────────────────────────────────────────────────
# DATABASE (Tracking-ready schema)
# ─────────────────────────────────────────────────────────
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS analyses (
            ...
            final_score INTEGER, final_verdict TEXT,
            royalty REAL, stressed_royalty REAL,
            product_name TEXT, created_at TEXT,
            published_date TEXT, day_30_bsr TEXT
        )
    """)
    conn.commit()
    conn.close()
...
# ─────────────────────────────────────────────────────────
# GEMINI DESIGNER (3.1 Flash Lite + Secrets Fallback)
# ─────────────────────────────────────────────────────────
...
{{
    ...
    palettes = [["#1A1A2E","#16213E","#E94560"],["#2D3436","#636E72","#FDCB6E"],["#0F3460","#533483","#E94560"],["#2C3E50","#27AE60","#F39C12"],["#1B1B2F","#E43F5A","#1F4068"]]
    return {
        "product_name": f"The {keyword.title()} Blueprint", "tagline": f"The only {keyword} built from real buyer feedback",
        "color_palette": random.choice(palettes), "cover_style": "Premium dark cover with minimalist typography and accent stripe",
        "key_features": [f"Solves: {fix[:60]}", "Undated format — start any day, any time", "Science-backed structure for lasting habits"],
...
def generate_listing(keyword, platform_id, design):
    ...
    if platform_id == "kdp":
        return {
            ...
            "description": f"Designed for {buyer.lower()}, {name} solves the #1 complaint buyers have with existing {base_kw} products. {tagline}. Undated format — start any day of the year.",
            "keywords": [base_kw, f"best {base_kw}", f"daily {base_kw}", f"{base_kw} gift", f"{base_kw} for women", f"{base_kw} for men", f"{base_kw} notebook", "undated journal", "mindfulness log"]
        }
...
def run_phase2(keyword):
    # Long-tail friendly threshold (Option 1)
    count = len(get_amazon_suggestions(keyword + " best seller", "aps"))
    score = 22 if count >= 8 else 16 if count >= 5 else 9
...
def run_phase3(keyword):
    ...
    if not complaints: complaints = [f"Pages feel flimsy in most {keyword} products", f"Prompts are too generic — not tailored for daily use", f"Cover wears out after a few weeks of daily use"]
...
# ─────────────────────────────────────────────────────────
# BATCH PROCESSOR (Streamlit-Compatible)
# ─────────────────────────────────────────────────────────
def run_batch_mode(keywords_text, platform_cfg, api_key=""):
    ...
    if len(keywords) > 15: return None, "Max 15 keywords per run to avoid Streamlit timeouts."
    ...
    for i, kw in enumerate(keywords):
        try:
            ...
            p1, p2, p3, p4 = run_phase1(kw, platform_cfg), run_phase2(kw), run_phase3(kw), run_phase4(platform_cfg["product"], platform_cfg)
            ...
            results.append({"keyword": kw, "score": final_score, "verdict": final_verdict, "product_name": design["product_name"], "title": listing["title"], "bullets": " | ".join(listing.get("bullets", [])), "description": listing.get("description", ""), "keywords": ",".join(listing.get("keywords", listing.get("tags", []))), "royalty": p4["royalty"], "stressed_margin": p4["stressed_margin"]})
            save_analysis(kw, platform_cfg["product"], final_score, final_verdict, p4["royalty"], p4["stressed_royalty"], design["product_name"])
        except Exception as e:
            errors.append(f"{kw}: {str(e)[:50]}")
        progress_bar.progress((i + 1) / len(keywords))

    status_text.text("✅ Batch complete!")
    progress_bar.empty()
    if errors: st.warning("⚠️ Some keywords failed:\n" + "\n".join(errors))
    return results, None
...
def main():
    ...
    <div style="text-align:center;padding:32px 0 16px">
      ...
      <p style="color:#6B8BA4;font-size:15px;max-width:480px;margin:0 auto;line-height:1.7">Enter a niche keyword. Get a GO/KILL signal, an AI-designed product, and a ready-to-publish listing — across KDP, MBA, and Etsy.</p>
    ...
    with st.sidebar:
        ...
        gemini_key = st.text_input("Gemini API Key", type="password", placeholder="AIza... (Stored in Secrets)", help="Key will be pulled from Streamlit Secrets automatically if left blank.")
        st.markdown("---")
        st.markdown("<div style='font-family:Space Mono,monospace;font-size:10px;color:#6B8BA4;line-height:1.8'><b style='color:#E8F4FF'>DATA SOURCES</b><br>✓ Amazon autocomplete<br>✓ Google autocomplete<br>✓ Reddit complaints<br>✓ Gemini 3.1 Flash Lite<br>✓ KDP / MBA / Etsy fee tables<br><br><b style='color:#E8F4FF'>COST</b><br>$0 — all free sources<br><br><b style='color:#E8F4FF'>PHASES</b><br>1 → Demand signal<br>2 → Market velocity<br>3 → Sentiment alpha<br>4 → Stress test</div>", unsafe_allow_html=True)

    col_kw, col_pl, col_btn = st.columns([3, 2, 1])
    with col_kw: keyword = st.text_input("Niche keyword", placeholder="e.g. gratitude journal, dog lover, daily planner 2026", label_visibility="collapsed")
    ...
    if batch_mode:
        ...
        if run_batch and batch_input.strip():
            ...
            if batch_results:
                ...
                st.download_button("📥 Download Results CSV", csv_data, file_name="quantdrop_batch_results.csv", mime="text/csv")
                st.dataframe(df[["keyword", "score", "verdict", "product_name"]])
    ...
    if run and keyword.strip() and not batch_mode:
        ...
        final_score = min(100, round(total / 95 * 100))
        final_verdict = "GO" if final_score >= 70 else "REVIEW" if final_score >= 40 else "KILL"
        progress.progress(100, text="Done!"); time.sleep(0.4); progress.empty()
        save_analysis(kw, platform_name, final_score, final_verdict, p4["royalty"], p4["stressed_royalty"], design["product_name"])

        verdict_msg = {"GO": "Validated — create and publish now", "REVIEW": "Promising — refine keyword before creating", "KILL": "Low signal — find a different niche"}.get(final_verdict, "—")
        vc = verdict_color(final_verdict)
        st.markdown(f"""<div class="verdict-{final_verdict.lower()}">
          <div style="display:flex;align-items:center;justify-content:space-between">
            <div><div style="font-family:Space Mono,monospace;font-size:24px;font-weight:700;color:{vc};letter-spacing:.06em">{verdict_emoji(final_verdict)} {final_verdict}</div>
            <div style="font-size:13px;color:#6B8BA4;margin-top:4px">{verdict_msg} · Platform: {platform_cfg['id'].upper()}</div></div>
            <div style="font-family:Space Mono,monospace;font-size:52px;font-weight:700;color:{vc}">{final_score} <span style="font-size:22px;color:#6B8BA4">/100</span></div>
          </div></div>""", unsafe_allow_html=True)

        st.markdown('<div class="section-header">Pipeline Scores</div>', unsafe_allow_html=True)
        pc1, pc2, pc3, pc4 = st.columns(4)
        for col, pnum, pname, pscore, pmax, pverdict in [(pc1,"Phase 1","Demand Signal",p1["score"],25,p1["verdict"]),(pc2,"Phase 2","Market Velocity",p2["score"],25,p2["verdict"]),(pc3,"Phase 3","Sentiment Alpha",p3["score"],25,p3["verdict"]),(pc4,"Phase 4","Stress Test",p4["score"],25,p4["verdict"])]:
        ...
        econ_col.markdown(f"""<div class="phase-card"><div style="font-family:Space Mono,monospace;font-size:10px;color:#6B8BA4;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px">Unit Economics — {platform_id.upper()}</div>
        <table style="width:100%;font-size:12px;border-collapse:collapse"><tr style="border-bottom:1px solid #1E2B38"><td style="color:#6B8BA4;padding:5px 0">Base royalty / sale</td><td style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;text-align:right">${p4['royalty']}</td></tr>
        <tr style="border-bottom:1px solid #1E2B38"><td style="color:#6B8BA4;padding:5px 0">Stressed royalty</td><td style="font-family:Space Mono,monospace;font-weight:700;color:#FFD166;text-align:right">${p4['stressed_royalty']}</td></tr>
        <tr style="border-bottom:1px solid #1E2B38"><td style="color:#6B8BA4;padding:5px 0">Base margin</td><td style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;text-align:right">{p4['margin']}%</td></tr>
        <tr style="border-bottom:1px solid #1E2B38"><td style="color:#6B8BA4;padding:5px 0">Stressed margin</td><td style="font-family:Space Mono,monospace;font-weight:700;color:{sm_color};text-align:right">{p4['stressed_margin']}%</td></tr>
        <tr><td style="color:#6B8BA4;padding:5px 0">Sales needed for $500/mo</td><td style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;text-align:right">{p4['monthly_sales_500']} sales</td></tr></table></div>""", unsafe_allow_html=True)
        ...
        sig_col.markdown(f"""<div class="phase-card"><div style="font-family:Space Mono,monospace;font-size:10px;color:#6B8BA4;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px">Demand Signals</div>
        <div style="font-size:12px;color:#6B8BA4;margin-bottom:6px;display:flex;align-items:center;gap:8px"><span style="min-width:160px">Amazon suggestions</span><div style="flex:1;height:4px;background:#1E2B38;border-radius:2px;overflow:hidden"><div style="height:100%;width:{a_pct}%;background:#7C3AED;border-radius:2px"></div></div><span style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;min-width:24px">{p1['amazon_count']}</span></div>
        <div style="font-size:12px;color:#6B8BA4;margin-bottom:6px;display:flex;align-items:center;gap:8px"><span style="min-width:160px">Google suggestions</span><div style="flex:1;height:4px;background:#1E2B38;border-radius:2px;overflow:hidden"><div style="height:100%;width:{g_pct}%;background:#00D4FF;border-radius:2px"></div></div><span style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;min-width:24px">{p1['google_count']}</span></div>
        <div style="font-size:12px;color:#6B8BA4;margin-bottom:6px;display:flex;align-items:center;gap:8px"><span style="min-width:160px">Velocity proxy</span><div style="flex:1;height:4px;background:#1E2B38;border-radius:2px;overflow:hidden"><div style="height:100%;width:{v_pct}%;background:#FF6B35;border-radius:2px"></div></div><span style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;min-width:24px">{p2['proxy_count']}</span></div>
        <div style="font-size:12px;color:#6B8BA4;margin-bottom:6px;display:flex;align-items:center;gap:8px"><span style="min-width:160px">Commercial maturity</span><div style="flex:1;height:4px;background:#1E2B38;border-radius:2px;overflow:hidden"><div style="height:100%;width:{min(100,p1['maturity']*33)}%;background:#FFD166;border-radius:2px"></div></div><span style="font-family:Space Mono,monospace;font-weight:700;color:#E8F4FF;min-width:24px">{p1['maturity']}</span></div></div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="design-card"><div style="font-family:Space Mono,monospace;font-size:9px;color:#00D4FF;letter-spacing:.12em;text-transform:uppercase;margin-bottom:8px">✦ AI DESIGNED BY GEMINI 3.1 FLASH LITE</div>
        <div style="font-size:26px;font-weight:800;color:#E8F4FF;margin-bottom:4px">{design.get('product_name', '—')}</div>
        <div style="font-size:13px;color:#6B8BA4;font-style:italic;margin-bottom:14px">"{design.get('tagline', '—')}"</div>
        <div style="margin-bottom:12px">{swatches}</div>
        <div style="font-family:Space Mono,monospace;font-size:9px;color:#6B8BA4;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Key Features</div>
        <ul style="margin:0;padding:0">{features_html}</ul>
        <div style="margin-top:12px;font-size:12px;color:#6B8BA4"><span style="color:#00D4FF;font-weight:600">Unique angle: </span>{design.get('unique_angle', '—')}</div>
        <div style="margin-top:6px;font-size:12px;color:#6B8BA4"><span style="color:#00D4FF;font-weight:600">Target buyer: </span>{design.get('target_buyer', '—')}</div>
        <div style="margin-top:8px;font-size:12px;color:#6B8BA4"><span style="color:#00D4FF;font-weight:600">Cover style: </span>{design.get('cover_style', '—')}</div></div>""", unsafe_allow_html=True)
        ...
        with st.expander("📋 Click to expand listing copy", expanded=True):
            ...
            if tags:
                ...
                st.markdown(f"""<div style="font-family:Space Mono,monospace;font-size:9px;color:#6B8BA4;text-transform:uppercase;letter-spacing:.08em;margin:12px 0 6px">{"Keywords" if platform_id == "kdp" else "Tags"}</div><div>{tags_html}</div>""", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

    elif run and not keyword.strip() and not batch_mode:
    ...
    st.markdown("""<div style="text-align:center;padding:32px 0 16px;border-top:1px solid #1E2B38;margin-top:40px"><div style="font-family:'Space Mono',monospace;font-size:11px;color:#6B8BA4">Built by a quant · Powered by <span style="color:#00D4FF">free data sources</span> + <span style="color:#FF6B35">Gemini 3.1 Flash Lite</span> · QuantDrop v2.0</div></div>""", unsafe_allow_html=True)
